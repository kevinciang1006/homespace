...
// PATCH: inline edit (on_hand, unit, and/or low_threshold). An on_hand
// change always logs a stock_movement — 'restock' when it went up (she
// topped it up), 'correction' when it went down (fixing a miscount; there's
// no consume/reserve flow yet in this layer). Amount is the signed delta so
// a future layer can just sum movements to reconcile on_hand. Editing only
// unit/low_threshold logs nothing — those aren't quantity changes.
